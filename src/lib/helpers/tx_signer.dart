...
class TransactionSigner {
  ...
  /// inside it.
  static Future<StdTx> signStdTx(Account account, StdTx stdTx,
      {String accountNumber = '', String sequence = ''}) async {
  ...
  /// inside it.
  static Future<VoteTx> signVoteTx(Account account, VoteTx voteTx,
      {String accountNumber = '', String sequence = ''}) async {
    // Get the account data and node info from the network
    if (accountNumber.isEmpty) {
      final CosmosAccount cosmosAccount = await QueryService.getAccountData(account);
    ...
    // Assemble the transaction
    return VoteTx(
        voteMsg: voteTx.voteMsg,
        authInfo: voteTx.authInfo,
        signatures: [signature['signature']],
        accountNumber: accountNumber,
        sequence: sequence);
  }

  static Map<String, dynamic> _getVoteSignature(Account account, String accountNumber, String sequence,
      NodeInfo nodeInfo, List<MsgVote> messages, StdFee fee, String memo) {
    // Create the signature object
